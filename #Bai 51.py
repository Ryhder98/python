...
class Ga:
    ...
    def sotrung(self):
        ...
        trunggaLai = 0
        trunggaKhac = 0
        if self.loai == 'Ri' and self.tuoi > 45:
            trunggaRi += 1
        elif self.loai == 'Lai' and self.tuoi > 30:
            trunggaLai += 2
        elif self.loai == 'Khac' and self.tuoi > 35:
            trunggaKhac += 1
        else:
            trunggaRi += 0
            trunggaLai += 0
            trunggaKhac += 0
        return trunggaRi, trunggaLai, trunggaKhac
def nhapsonguyen():
    while True:
...
def hienthi(listga,n):
    print('{:<15}{:<15}{:<15}{:<15}{:<15}'.format('Loai','Tuoi','Gioi tinh','So luong','So trung'))
    for i in range(n):
        print('{:<15}{:<15}{:<15}{:<15}{:<15}'.format(listga[i].loai,listga[i].tuoi,listga[i].gioitinh,listga[i].sl,listga[i].sotrung()))
def tongsotrung(listga, n):
    tong = 0
    for i in range(n):
        tong += listga[i].sotrung()
    return tong
